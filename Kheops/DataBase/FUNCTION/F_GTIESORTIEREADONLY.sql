CREATE FUNCTION ZALBINKHEO.F_GTIESORTIEREADONLY ( 
	...
	RETURNS VARCHAR(12)   
	LANGUAGE SQL 
	SPECIFIC ZALBINKHEO.F_GTIESORTIEREADONLY 
	NOT DETERMINISTIC 
	READS SQL DATA 
	...
	SELECT COUNT ( * ) INTO V_COUNTRSQ FROM ZALBINKHEO . KPOPTAP WHERE TRIM ( KDDIPB ) = TRIM ( P_CODEOFFRE ) AND KDDALX = P_VERSION AND KDDTYP = P_TYPE AND KDDFOR = P_CODEFORMULE AND KDDOPT = P_CODEOPTION AND KDDPERI = 'RQ' ; 
	SELECT COUNT ( * ) INTO V_COUNTOBJ FROM ZALBINKHEO . KPOPTAP WHERE TRIM ( KDDIPB ) = TRIM ( P_CODEOFFRE ) AND KDDALX = P_VERSION AND KDDTYP = P_TYPE AND KDDFOR = P_CODEFORMULE AND KDDOPT = P_CODEOPTION AND KDDPERI = 'OB' ; 
		...
		IF ( V_NATURE = 'E' ) THEN  --DÉTERMINATION DE LA PLAGE DES OBJETS INCLUS 
			...
			IF ( V_COUNTOBJ > 0 ) THEN 
				...
				FROM ZALBINKHEO . YPRTOBJ Y 
					INNER JOIN ZALBINKHEO . KPOPTAP O ON Y . JGIPB = O . KDDIPB AND Y . JGALX = O . KDDALX AND Y . JGRSQ = O . KDDRSQ AND Y . JGOBJ = O . KDDOBJ 
					INNER JOIN ZALBINKHEO . KPOPT OP ON OP . KDBID = O . KDDKDBID 
					INNER JOIN ZALBINKHEO . KPOPTD OPD ON OPD . KDCKDBID = OP . KDBID 
					INNER JOIN ZALBINKHEO . KPGARAN GAR ON GAR . KDEKDCID = OPD . KDCID 
	...
	ELSE  -- SI PAS DE PORTÉE DE GARANTIE 
		IF ( V_COUNTRSQ > 0 ) THEN 
			SELECT IFNULL ( MIN ( JGVDA * 100000000 + JGVDM * 1000000 + JGVDJ * 10000 + JGVDH ) , 0 ) INTO V_DATEMIN 
			FROM ZALBINKHEO . YPRTOBJ Y 
				INNER JOIN ZALBINKHEO . KPOPTAP O ON Y . JGIPB = O . KDDIPB AND Y . JGALX = O . KDDALX AND Y . JGRSQ = O . KDDRSQ 
				INNER JOIN ZALBINKHEO . KPOPT OP ON OP . KDBID = O . KDDKDBID 
				INNER JOIN ZALBINKHEO . KPOPTD OPD ON OPD . KDCKDBID = OP . KDBID 
				INNER JOIN ZALBINKHEO . KPGARAN GAR ON GAR . KDEKDCID = OPD . KDCID 
			WHERE GAR . KDEID = P_IDGAR ; 
	...
	IF ( V_DATEMIN = 0 ) THEN 
		...
		FROM ZALBINKHEO . YPRTRSQ Y 
			INNER JOIN ZALBINKHEO . KPOPTAP O ON Y . JEIPB = O . KDDIPB AND Y . JEALX = O . KDDALX AND Y . JERSQ = O . KDDRSQ 
			INNER JOIN ZALBINKHEO . KPOPT OP ON OP . KDBID = O . KDDKDBID 
			INNER JOIN ZALBINKHEO . KPOPTD OPD ON OPD . KDCKDBID = OP . KDBID 
			INNER JOIN ZALBINKHEO . KPGARAN GAR ON GAR . KDEKDCID = OPD . KDCID 
		WHERE GAR . KDEID = P_IDGAR ; 
