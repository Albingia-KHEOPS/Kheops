CREATE PROCEDURE SP_GETGARANTIEDETAILS_READONLY ( 
	...
	LANGUAGE SQL 
	SPECIFIC SP_GETGARANTIEDETAILS_READONLY 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
...
WHERE KDEIPB = P_CODEOFFRE AND KDEALX = P_VERSION AND KDETYP = P_TYPE AND KDEID = P_IDGAR ;	 
	...
	SELECT PBPER INTO V_PERIODICITE FROM YPOBASE WHERE PBIPB = P_CODEOFFRE AND PBALX = P_VERSION AND PBTYP = P_TYPE ; 
		 
	SELECT DISTINCT JETEM INTO V_RSQTEMP 
		FROM YPRTRSQ 
			INNER JOIN KPOPTAP ON KDDIPB = JEIPB AND KDDALX = JEALX AND KDDRSQ = JERSQ AND KDDFOR = P_CODEFORMULE AND KDDOPT = P_CODEOPTION 
	...
	SELECT COUNT ( * ) INTO V_COUNTRSQ FROM KPOPTAP WHERE KDDIPB = P_CODEOFFRE AND KDDALX = P_VERSION AND KDDTYP = P_TYPE AND KDDFOR = P_CODEFORMULE AND KDDOPT = P_CODEOPTION AND KDDPERI = 'RQ' ; 
	SELECT COUNT ( * ) INTO V_COUNTOBJ FROM KPOPTAP WHERE KDDIPB = P_CODEOFFRE AND KDDALX = P_VERSION AND KDDTYP = P_TYPE AND KDDFOR = P_CODEFORMULE AND KDDOPT = P_CODEOPTION AND KDDPERI = 'OB' ; 
	...
	IF ( V_COUNT > 0 ) THEN  --SI PORTÉE DE GARANTIE 
		...
		IF ( V_NATURE = 'E' ) THEN  --DÉTERMINATION DE LA PLAGE DES OBJETS INCLUS 
			IF ( V_COUNTRSQ > 0 ) THEN 
				...
				SELECT IFNULL ( MIN ( JGVDA * 10000000000 + JGVDM * 100000000 + JGVDJ * 1000000 + JGVDH * 100 ) , 0 ) , 
					IFNULL ( MAX ( JGVFA * 10000000000 + JGVFM * 100000000 + JGVFJ * 1000000 + JGVFH * 100 ) , 0 ) INTO V_DATEMIN , V_DATEMAX 
				FROM YPRTOBJ Y WHERE JGIPB = P_CODEOFFRE AND JGALX = P_VERSION AND NOT EXISTS ( SELECT * FROM KPGARAP G WHERE G . KDFKDEID = P_IDGAR 
			...
			IF ( V_COUNTOBJ > 0 ) THEN 
				...
				SELECT IFNULL ( MIN ( JGVDA * 10000000000 + JGVDM * 100000000 + JGVDJ * 1000000 + JGVDH * 100 ) , 0 ) , 
					IFNULL ( MAX ( JGVFA * 10000000000 + JGVFM * 100000000 + JGVFJ * 1000000 + JGVFH * 100 ) , 0 ) INTO V_DATEMIN , V_DATEMAX 
				FROM YPRTOBJ Y 
					INNER JOIN KPOPTAP O ON Y . JGIPB = O . KDDIPB AND Y . JGALX = O . KDDALX AND Y . JGRSQ = O . KDDRSQ AND Y . JGOBJ = O . KDDOBJ 
					INNER JOIN KPOPT OP ON OP . KDBID = O . KDDKDBID 
					INNER JOIN KPOPTD OPD ON OPD . KDCKDBID = OP . KDBID 
					INNER JOIN KPGARAN GAR ON GAR . KDEKDCID = OPD . KDCID 
	...
	ELSE  -- SI PAS DE PORTÉE DE GARANTIE 
		IF ( V_COUNTRSQ > 0 ) THEN 
			...
			FROM YPRTRSQ Y 
				INNER JOIN KPOPTAP O ON Y . JEIPB = O . KDDIPB AND Y . JEALX = O . KDDALX AND Y . JERSQ = O . KDDRSQ 
				INNER JOIN KPOPT OP ON OP . KDBID = O . KDDKDBID 
				INNER JOIN KPOPTD OPD ON OPD . KDCKDBID = OP . KDBID 
				INNER JOIN KPGARAN GAR ON GAR . KDEKDCID = OPD . KDCID 
			WHERE GAR . KDEID = P_IDGAR ;  -- AND ( JEVFA * 100000000 + JEVFM * 1000000 + JEVFJ * 10000 + JEVFH ) > 0 ; 
		END IF ; 
		IF ( V_COUNTOBJ > 0 ) THEN 
			SELECT IFNULL ( MIN ( JGVDA * 10000000000 + JGVDM * 100000000 + JGVDJ * 1000000 + JGVDH * 100 ) , 0 ) , 
				IFNULL ( MAX ( JGVFA * 10000000000 + JGVFM * 100000000 + JGVFJ * 1000000 + JGVFH * 100 ) , 0 ) INTO V_DATEMIN , V_DATEMAX 
