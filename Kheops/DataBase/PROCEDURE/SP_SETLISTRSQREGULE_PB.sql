...
CREATE PROCEDURE  SP_SETLISTRSQREGULE_PB ( 
	...
	LANGUAGE SQL 
	SPECIFIC SP_SETLISTRSQREGULE_PB 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
	...
	P1 : BEGIN ATOMIC 
		...
		DECLARE V_RIA DECIMAL ( 3 , 0 ) DEFAULT 0 ; 
		...
		ELSE 
			...
			IF ( V_RGID = 0 ) THEN 
				...
				SELECT ( PBAVA * 10000 + PBAVM * 100 + PBAVJ ) , JDAFC 
					INTO V_DATEAVN , V_APPLFRAISACC
				FROM YPOBASE 
					INNER JOIN YPRTENT ON PBIPB = JDIPB AND PBALX = JDALX 
				WHERE PBIPB = P_CODECONTRAT AND PBALX = P_VERSION AND PBTYP = P_TYPE AND PBAVN = P_CODEAVN ; 
				 
				 -- Pas utile depuis la séparation des Régule/PB/BNS
				...
				IF ( P_REGULNIV = 'E' ) THEN 
					...
					DO 
						...
						VALUES 
							...
							P_REGULAVN , P_CODEAVN , V_DATEAVN , P_EXERCICE , P_PERIODEDEB , 
			...
			( P_TYPE , P_CODECONTRAT , P_VERSION , P_CODEAVN , P_REGULMODE , 'G' , V_DATENOWANNEE , V_DATENOWMOIS , V_DATENOWJOUR , V_HEURENOW , 
				'PB - Participation bénéfices' , 'I' , 0 , 0 , 0 , 0 , 0 , 0 , P_USER , V_DATENOWANNEE , V_DATENOWMOIS , V_DATENOWJOUR , V_HEURENOW ) ; 
