...
namespace Albingia.Kheops.OP.DataAdapter.DataModel.Repository {
    ...
    public  partial class  HpirsgaRepository : BaseTableRepository {
...
, KFDAN07, KFDCLAL FROM HPIRSGA
WHERE KFDTYP = :type
and KFDIPB = :numeroAffaire
and KFDALX = :numeroAliment
and KFDAVN = :numeroAvenant
";
            const string update=@"UPDATE HPIRSGA SET 
...
, KFDAN07 = :KFDAN07, KFDCLAL = :KFDCLAL
 WHERE 
KFDTYP = :type and KFDIPB = :numeroAffaire and KFDALX = :numeroAliment and KFDAVN = :numeroAvenant";
            const string delete=@"DELETE FROM HPIRSGA WHERE KFDTYP = :type AND KFDIPB = :numeroAffaire AND KFDALX = :numeroAliment AND KFDAVN = :numeroAvenant";
            const string insert=@"INSERT INTO  HPIRSGA (
KFDTYP, KFDIPB, KFDALX, KFDAVN, KFDHIN
, KFDFOR, KFDOPT, KFDCRD, KFDCRH, KFDMAJU
, KFDMAJD, KFDMAJH, KFDAN02, KFDAN03, KFDBO01
, KFDBO02, KFDBO03, KFDIM08, KFDIM09, KFDIM10
...
";
            ...
            public HpirsgaRepository (IDbConnection connection, IdentifierGenerator idGenerator) : base (connection) {
                this.idGenerator = idGenerator;
            }
 
            public KpIrSGa Get(string type, string numeroAffaire, int numeroAliment, int numeroAvenant){
                return connection.Query<KpIrSGa>(select, new {type, numeroAffaire, numeroAliment, numeroAvenant}).SingleOrDefault();
            }
            ...
            public void Delete(KpIrSGa value){
                    var parameters = new DynamicParameters();
                    parameters.Add("type",value.Kfdtyp);
                    parameters.Add("numeroAffaire",value.Kfdipb);
                    parameters.Add("numeroAliment",value.Kfdalx);
                    parameters.Add("numeroAvenant",value.Kfdavn);
            ...
            public IEnumerable<KpIrSGa> GetByAffaire(string type, string code, int version, int numeroAvenant){
                    return connection.EnsureOpened().Query<KpIrSGa>(select_GetByAffaire, new {type, code, version, numeroAvenant}).ToList();
            }
    }
}
