...
namespace Albingia.Kheops.OP.DataAdapter.DataModel.Repository {
    ...
    public  partial class  HpirsobRepository : BaseTableRepository {
...
, KFASURF, KFAVMC, KFAPROL, KFADEPD FROM HPIRSOB
WHERE KFATYP = :KFATYP
and KFAIPB = :KFAIPB
and KFAALX = :KFAALX
and KFAAVN = :KFAAVN
...
and KFAOBJ = :KFAOBJ
";
            const string update=@"UPDATE HPIRSOB SET 
...
, KFATU1 = :KFATU1, KFATU2 = :KFATU2, KFAASC = :KFAASC, KFAAUTL = :KFAAUTL, KFAQMD = :KFAQMD, KFASURF = :KFASURF, KFAVMC = :KFAVMC, KFAPROL = :KFAPROL, KFADEPD = :KFADEPD
 WHERE 
KFATYP = :KFATYP and KFAIPB = :KFAIPB and KFAALX = :KFAALX and KFAAVN = :KFAAVN and KFAHIN = :KFAHIN and KFARSQ = :KFARSQ and KFAOBJ = :KFAOBJ";
            const string delete=@"DELETE FROM HPIRSOB WHERE KFATYP = :KFATYP AND KFAIPB = :KFAIPB AND KFAALX = :KFAALX AND KFAAVN = :KFAAVN AND KFAHIN = :KFAHIN AND KFARSQ = :KFARSQ AND KFAOBJ = :KFAOBJ";
            const string insert=@"INSERT INTO  HPIRSOB (
KFATYP, KFAIPB, KFAALX, KFAAVN, KFAHIN
, KFARSQ, KFAOBJ, KFANATS, KFANEGA, KFAFRQE
, KFANBPA, KFANBEX, KFANBVI, KFAGN08, KFAGN09
, KFAGN10, KFANBIN, KFANBPE, KFAGCT, KFANBEM
...
";
            ...
            public HpirsobRepository (IDbConnection connection, IdentifierGenerator idGenerator) : base (connection) {
                this.idGenerator = idGenerator;
            }
 
            public KpIrsOb Get(string KFATYP, string KFAIPB, int KFAALX, int KFAAVN, int KFAHIN, int KFARSQ, int KFAOBJ){
                return connection.Query<KpIrsOb>(select, new {KFATYP, KFAIPB, KFAALX, KFAAVN, KFAHIN, KFARSQ, KFAOBJ}).SingleOrDefault();
            }
            ...
            public void Delete(KpIrsOb value){
                    ...
                    parameters.Add("KFAAVN",value.Kfaavn);
                    parameters.Add("KFAHIN",value.Kfahin);
            ...
            public IEnumerable<KpIrsOb> GetByAffaire(string KFATYP, string KFAIPB, int KFAALX, int KFAAVN){
                    return connection.EnsureOpened().Query<KpIrsOb>(select_GetByAffaire, new {KFATYP, KFAIPB, KFAALX, KFAAVN}).ToList();
            }
    }
}
